...
const Videos = () => {
    ...
    const videosArr = [
      ...
      const [headingText, setHeadingText] = useState('Lecture 1');
      const [description, setDescription] = useState('Description of Lecture 1');

  const changeVideo = (item, index) => {
    setVideoSrc(item);
    setHeadingText(`Lecture ${index + 1}`);
    setDescription(`Description of Lecture ${index + 1}`);
  };
    ...
    <Stack direction={['column', 'row']} h={'100vh'}>
        ...
        {videosArr.map((item, index) => (
          <Button
            ...
            variant={'ghost'}
            colorScheme={'purple'}
            onClick={() => changeVideo(item, index)}
          >
            Lecture {index + 1}
          </Button>
        ))}
